...
describe("[Challenge] Puppet v2", function () {
  ...
  before(async function () {
    ...
    uniswapRouter = await UniswapRouterFactory.deploy(
      uniswapFactory.address, // 0x000...0
      weth.address
    );
    ...
    ).to.eq(300000n * 10n ** 18n); // 300k WETH
  });
  ...
  it("Execution", async function () {
    ...
    await weth
      .connect(player)
      .approve(lendingPool.address, PLAYER_INITIAL_ETH_BALANCE * 2n);
    ...
    const playerETHBalance = (
      await ethers.provider.getBalance(player.address)
    ).toBigInt();

    await weth
      .connect(player)
      .deposit({ value: playerETHBalance - 100000000000000000n });
