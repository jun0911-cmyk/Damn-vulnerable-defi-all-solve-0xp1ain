...
contract TheRewarderPool {
    ...
    function distributeRewards() public returns (uint256 rewards) {
        if (isNewRewardsRound()) {
            // @audit-info save the last snapshot id for rewards
            _recordSnapshot();
        }

        // @audit-info totalDeposits = accountingToken total supply at lastSnapshotIdForRewards
        // @audit-info amountDeposited = accountingToken balance of at msg sender lastSnapshotIdForRewards
        uint256 totalDeposits = accountingToken.totalSupplyAt(lastSnapshotIdForRewards);
        uint256 amountDeposited = accountingToken.balanceOfAt(msg.sender, lastSnapshotIdForRewards);

        if (amountDeposited > 0 && totalDeposits > 0) {
            // @audit-info rewards = amountDeposited * REWARDS / totalDeposits
    ...
    function isNewRewardsRound() public view returns (bool) {
        // @audit-info last recorded snapshot timestemp + 5 days <= block timestamp
